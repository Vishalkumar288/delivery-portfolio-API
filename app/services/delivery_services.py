...
def get_high_level_data():
    ...
    for index, row in enumerate(raw_data, start=1):
        clean = {k.lower().replace(" ", "_").replace("%", "percent"): v for k, v in row.items()}
        ...
        transformed.append({
            ...
            "eta": format_date_to_iso(clean.get("eta")),
...
def get_sorted_risks(
    ...
    risk_data = [m for m in filtered if m.get("raw_status", "").lower() in ("red", "amber")]
...
def get_project_details(target_project: str):
    ...
    project_main = next((r for r in detailed if str(r.get("Project Name")).lower().replace(" ", "-") == slug), None)
    ...
    return {
        ...
        "milestones": [
            ...
            for m in milestone_rows if m.get("Project") == actual_name
        ]
    }
...
def get_delivery_filter_options():
    ...
    def format_opt(s):
        return sorted(
            [{"name": v, "displayName": v} for v in s], 
